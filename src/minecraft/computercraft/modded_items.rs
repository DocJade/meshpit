...
fn check_modded_items() {
    let mut modded_values: Vec<Item> = MODDED_ITEMS.clone().into_values().collect();
    // yeah idk what this is either
    let mut vanilla_values: Vec<Item> = <std::collections::HashMap<
        std::string::String,
        mcdata_rs::Item,
    > as Clone>::clone(&get_mc_data().items_by_name)
    .into_values()
    .collect();
    ...
    let mut combined: Vec<Item> = Vec::with_capacity(vanilla_dedup.len() + modded_dedup.len());
